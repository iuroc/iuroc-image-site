...
const ImageBox = (src: string) => {
    return div({ class: 'col-xl-3 col-lg-4 col-6 mb-4' },
        div({ class: 'position-relative starRouteImage', role: 'button' },
            div({ class: 'ratio ratio-16x9' },
                img({ src, class: 'w-100 h-100 rounded-4' }),
            ),
            starIcon()
        )
    )
...
export const starIcon = () => div({ class: 'icon position-absolute', role: 'button', style: 'bottom: 10px; right: 10px;' },
